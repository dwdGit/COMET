CREATE OR REPLACE PROCEDURE "C##COMET".GENERA_PRODUZIONE(
	p_CFDipendente		IN DIPENDENTE.CODICEFISCALE%TYPE  
) IS 
	TYPE prod_type IS RECORD (
		codice_prodotto 		PRODOTTOFINITO.CODICEPRODOTTO%TYPE,
	...
	CURSOR c_pf_prod IS
		...
		FROM FORMULA f 
		JOIN (
			SELECT CODICEPRODOTTO, (QUANTITARIMANENTE * -1) AS QTA_DA_PRODURRE  
			FROM INVENTARIO_PRODOTTI_FINITI 
			WHERE QUANTITARIMANENTE < 0
		) qdp ON qdp.CODICEPRODOTTO = f.CODICEPRODOTTOFINITO;
...
BEGIN 
	SELECT COUNT(*)
	INTO count_dipendente
	FROM DIPENDENTE
	WHERE CODICEFISCALE = p_CFDipendente
	AND MANSIONE = 'Produzione'
	AND CFSUPERVISORE IS NULL;

	IF count_dipendente = 0 THEN
		RAISE DIPENDENTE_NON_ABILITATO;
	END IF;
	...
	EXIT WHEN c_pf_prod%NOTFOUND;
		...
		INTO codice_formula
		FROM FORMULA 
		WHERE CODICEPRODOTTOFINITO = prod_record.codice_prodotto;
...
EXCEPTION
	WHEN DIPENDENTE_NON_ABILITATO THEN
		RAISE_APPLICATION_ERROR(-20022, 'L''utentente non è abilitato all''esecuzione di questa operazione.');
END; 